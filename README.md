...
- **交互融合**（可选，默认关闭）：开启后弹窗内容融合到 ClassIsland 主界面覆盖层显示（无边框、贴合主界面），而非独立窗口
...
- **交互融合**（可选，默认关闭）：在「详细设置 → 弹窗行为」中开启后，弹窗内容融合到 ClassIsland 主界面覆盖层显示，而非独立窗口
...
- 自动执行（倒计时后自动触发指定按钮）
- **交互融合**：在「详细设置 → 弹窗行为」中开启，用主界面覆盖层显示而非独立窗口
